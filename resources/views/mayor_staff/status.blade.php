...
<div id="intakeSheetModal" class="modal">
    <div class="modal-content">
        ...
        <div id="reviewArea" class="review-columns" style="max-height: 80vh; overflow-y: auto;">
            ...
            <!-- Documents Section - AT THE BOTTOM AND ALREADY OPEN/EXPANDED -->
 <!-- Documents Section - AT THE BOTTOM AND ALREADY OPEN/EXPANDED -->
<div class="intake-section">
...
function openIntakeSheetModal(id, name, type = 'intake') {
    ...
    // Fetch intake sheet data using the correct endpoint
...
function populateIntakeSheetModal(data, type = 'intake') {
    ...
    if (data.social_service_records && data.social_service_records.length > 0) {
        data.social_service_records.forEach(record => {
            const row = document.createElement('tr');
            row.innerHTML = `
                <td>${record.date || '-'}</td>
    ...
    workerSignatureDiv.innerHTML = data.signature_worker ? 
        `<img src="${data.signature_worker}" alt="Worker Signature" style="max-width:180px;height:60px;object-fit:contain;">` : 
        '<div style="height:60px;"></div>';
        
    officerSignatureDiv.innerHTML = data.signature_officer ? 
        `<img src="${data.signature_officer}" alt="Officer Signature" style="max-width:180px;height:60px;object-fit:contain;">` : 
        '<div style="height:60px;"></div>';
        
    clientSignatureDiv.innerHTML = data.signature_client ? 
        `<img src="${data.signature_client}" alt="Client Signature" style="max-width:300px;height:80px;object-fit:contain;">` : 
    ...
    Swal.fire({
        ...
        if (result.isConfirmed && result.value) {
            ...
            .then(data => {
                ...
                // FIX: Properly check the response
        ...
        if (result.isConfirmed) {
            ...
            .then(data => {
                ...
                if (data.success === true) {
                    ...
                    // If success is false or undefined, show error
