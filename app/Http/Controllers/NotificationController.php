...
class NotificationController extends Controller
{
    ...
    {
        $user = Auth::user();
        return $user && in_array($user->role, ['lydo_admin', 'mayor_staff']);
    }
    ...
    public function sendBulkSms(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), [
            ...
            'applications.*.applicant_id' => 'required|integer',
            'applications.*.applicant_contact_number' => 'required|string',
            'message' => 'required|string|max:160',
        ]);
        ...
        try {
            ...
            foreach ($applications as $application) {
                $mobile = $application['applicant_contact_number'];
                if (!empty($mobile)) {
                    // TODO: Integrate with actual SMS service
            ...
            return $this->successResponse([
                ...
                'message' => 'Bulk SMS sending not yet implemented - would send to ' . $sentCount . ' recipients'
            ], 'Bulk SMS queued successfully');

        } catch (\Exception $e) {
            return $this->errorResponse('Failed to send bulk SMS: ' . $e->getMessage(), 500);
        }
    }
